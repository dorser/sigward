...
CONTAINER_REPO ?= ghcr.io/micromize-dev/sigward
IMAGE_TAG ?= $(TAG)
...
.PHONY: license-check
license-check:
	@go run github.com/google/addlicense@v1.2.0 -check -l apache -c "The Sigward authors" \
		$$(find . -name '*.go' -not -path './build/*')

.PHONY: license-add
license-add:
	@go run github.com/google/addlicense@v1.2.0 -y "" -l apache -c "The Sigward authors" \
		$$(find . -name '*.go' -not -path './build/*')
...
$(GOARCHS):
	...
	cp build/gadgets/*.tar build/src/cmd/sigward/build/
...
.PHONY: run-sigward
run-sigward:
	sudo -E ig run $(CONTAINER_REPO)/sigward:$(IMAGE_TAG) $$PARAMS
...
ifeq ($(strip $(DEV_REGISTRY)),)
	...
	docker build --no-cache -t $(DEV_REGISTRY)/sigward:$(DEV_TAG) .
...
ifeq ($(strip $(DEV_REGISTRY)),)
	...
	docker push $(DEV_REGISTRY)/sigward:$(DEV_TAG)
...
ifeq ($(strip $(DEV_REGISTRY)),)
	...
	helm upgrade --install sigward ./charts/sigward \
		-n $(DEV_NAMESPACE) --create-namespace \
		--set image.repository=$(DEV_REGISTRY)/sigward \
		--set image.tag=$(DEV_TAG) \
		--set image.pullPolicy=Always \
		--set logLevel=debug \
		--set filterNamespaces="default\,dev" \
		$(DEV_HELM_ARGS)
	kubectl rollout restart daemonset sigward -n $(DEV_NAMESPACE)
	kubectl rollout status daemonset sigward -n $(DEV_NAMESPACE) --timeout=120s
...
dev-logs: ## Tail logs from all dev pods
	kubectl logs -n $(DEV_NAMESPACE) -l app.kubernetes.io/name=sigward -f --prefix --all-containers

.PHONY: dev-status
dev-status: ## Show dev pod status
	kubectl get pods -n $(DEV_NAMESPACE) -l app.kubernetes.io/name=sigward -o wide
