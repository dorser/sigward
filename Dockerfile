...
WORKDIR /app
COPY include/sigward /usr/include/sigward
...
RUN mkdir -p build/gadgets && \
    for gadget in sigward; do \
        ...
        ig image build \
            --local \
            -t ghcr.io/micromize-dev/sigward/${gadget}:${IMAGE_TAG} \
            gadgets/${gadget} && \
        ig image export \
            ghcr.io/micromize-dev/sigward/${gadget}:${IMAGE_TAG} \
...
# They need to be placed where embeds.go expects them (cmd/sigward/build/)
RUN mkdir -p cmd/sigward/build
COPY --from=gadget-builder /app/build/gadgets/*.tar cmd/sigward/build/
